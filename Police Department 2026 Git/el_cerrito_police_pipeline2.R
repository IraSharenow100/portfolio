...
#             correction found during the FY 2025-26 update. Results
#             marked "published" in Phases 8-17 are pre-7B values.
#
...
# correctly -- but for 291 addresses (1,499 records) it placed the
...
#   "6400 CUTTING BLVD, SAN PABLO AVE & KEARNEY ST, EL CERRITO, CA, 94530"
# Phase 4 geocoded that whole string. For most, ArcGIS placed the house
# correctly -- but for some it placed the cross-street intersection
# instead. Worst case: the Del Norte BART station, placed 3,437 ft from
# the station.
#
# FIX: the FY 2025-26 pass (Phase B4) geocoded house addresses with the
# cross-street removed. Where the same house appears in both periods and
...
# Result: TBD (first version: 16442 records, 3173 addresses, median 0 ft,
# 9.1% over 500 ft, 5.8% over 1,000 ft)
...
nrow(misplaced)                                                  # TBD (first version: 291)
sum(misplaced$n)                                                 # TBD (first version: 1499)
round(100 * sum(head(misplaced$n, 10)) / sum(misplaced$n), 1)    # TBD (first version: 42.5)
...
sum(misplaced$address %in% outliers_v3_addresses)  # expect 0 -- excluded above
...
nrow(police_geocoded)                  # expect 137194
sum(!police_geocoded$BAD_LATLON_FLAG)  # expect 132996 -- correction moves points, doesn't add or drop any
...
# Result after 7B: TBD
# Published (pre-7B): EC Plaza BART 2298, Community Center 1802, ECHS 1235,
# Library 1029, Harding 786, Del Norte BART 557, Madera 525, Castro Park
# 364, Korematsu 294.
#
...
outliers_v3 <- read_csv(file.path(output_dir, "geocode_outliers_v3.csv"), show_col_types = FALSE)

misplaced %>%
  filter(address %in% outliers_v3$address) %>%
  left_join(outliers_v3 %>% select(address, lat_v3 = lat, long_v3 = long), by = "address") %>%
  mutate(
    part_a_mi  = round(distHaversine(cbind(long, lat),       c(EC_CENTER_LONG, EC_CENTER_LAT)) / 1609.34, 2),
    fy26_mi    = round(distHaversine(cbind(long_b, lat_b),   c(EC_CENTER_LONG, EC_CENTER_LAT)) / 1609.34, 2),
    phase10_mi = round(distHaversine(cbind(long_v3, lat_v3), c(EC_CENTER_LONG, EC_CENTER_LAT)) / 1609.34, 2),
    fy26_vs_phase10_ft = round(distHaversine(cbind(long_b, lat_b), cbind(long_v3, lat_v3)) / 0.3048)
  ) %>%
  select(address, n, part_a_mi, fy26_mi, phase10_mi, fy26_vs_phase10_ft) %>%
  as.data.frame()
...
nrow(outliers_full)  # result after 7B: TBD (published, pre-7B: 302)
...
nrow(police_total_corrected)  # result after 7B: TBD (published, pre-7B: 37867)
...
# Published (pre-7B): 37867 / 2887 / 1164 / 33816
...
landmark_comparison
# Result after 7B: TBD
# Published (pre-7B): EC Plaza BART and EC Community Center led in BOTH
# vehicle crime and dangerous driving. Korematsu/Madera lowest across all
# three. Re-check -- Del Norte BART and the Community Center are the
# landmarks most affected by 7B.

# ============================================================
# PHASE 13 -- VEHICLE-CRIME HEATMAP
# ============================================================

nrow(police_vehicle_crime)  # published (pre-7B): 2887
...
# EC Plaza BART). Re-check after 7B.
...
# Result after 7B: TBD
# Published (pre-7B): the SHARE near the Plaza (1,000 ft) has not risen --
# 10.4% (2019) to 6.7% (2025), if anything lower now. The raw count near
# the Plaza grew, but that's just riding the citywide tripling, not extra
# concentration there. Enforcement growth is broad-based.
...
# Result after 7B: TBD
# Pre-7B comparison: Del Norte BART 359 vs 86/yr (Part A undercount, see
# 7B), EC Plaza BART 343 vs 354, Library 248 vs 158, Community Center 247
# vs 277, ECHS 176 vs 190, Harding 161 vs 121, Castro Park 54 vs 56,
# Madera 48 vs 81, Korematsu 44 vs 45.
...
near_del_norte %>%
  ...
  count(call_for_service, sort = TRUE) %>%
  print(n = 10)
# Result: TBD -- expect parking to be most of the crime-scope count here
...
parking_fy
# Result: TBD. Preliminary: FY 2024-25 ~2317, FY 2025-26 1931 (down ~17%).
...
parking_plaza_fy26
# Result: TBD. Compare to Part A's yearly range (Phase 17, after 7B).
...
# --- CHECK: every address form that mentions the greenway ---
police_fy26 %>%
  filter(str_detect(address, "OHLONE")) %>%
  count(street, sort = TRUE) %>%
  print(n = 100)
# Result: 97 distinct forms -- "OHLONE GREENWAY" (99), intersections with
# OHLONE TRL / OHLONE GREENWAY, shorthand ("WALL/OHLONE", "POT/OHLONE"),
# and house addresses ("6543 PORTOLA DR") where the greenway appears only
# as a cross-street note.
