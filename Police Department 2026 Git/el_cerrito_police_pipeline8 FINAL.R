...
#
#   PART B -- FY 2025-26: July 1, 2025 through June 30, 2026 (Phases B0-B16)
...
#                       for the greenway, which runs alongside the BART tracks
...
greenway_monthly %>%
  ...
  print(n = 36)
# Result: security checks on the greenway averaged ~44/month in 2019, fell
# to 11 and 13 in Jan-Feb 2020 -- BEFORE the mid-March 2020 COVID shutdown
# -- and stayed at 2-14/month through 2021. Other officer-initiated
# activity: up to 20/month in late 2019, 0-4/month from 2020 on.
...
police_clean_dated %>%
  ...
  print(n = 36)
# Result: greenway-specific. Citywide checks did not fall in early 2020
# (338 Jan, 293 Feb; 2019 range ~200-480) and JUMPED at the COVID start
# (1,269 Mar 2020, 747 Apr). The greenway's share fell from ~11% (2019) to
# 1-5% from Jan 2020 on, through 2021. The data can't say why.
...
category_fy
# Result (FY 2019-20 ... FY 2025-26):
#   Crimes against persons 597 431 504 499 560 502 418
#   Property crime        2016 1469 1540 1271 1307 1164 976
#   Dangerous driving      309 297 299 266 305 288 245
#   Disorder              2352 1998 1901 2074 1878 1842 1667
#   Parking                702 610 863 1054 1519 2317 1931
#   Officer-initiated     9190 6302 4241 5221 7530 8762 8289
#   Everything else       7849 6857 7141 7626 10068 10179 9592
# Persons, property, disorder, dangerous driving all at 7-year lows in
# FY 2025-26. These are calls/reports logged, not confirmed crimes.
...
police_all %>%
  ...
  pivot_wider(names_from = fy_label, values_from = n, values_fill = 0) %>%
  print(n = 20)
# Result: assault 235 -> 171 (flat last two years); robbery & carjacking
# 48 -> 15 in FY 2025-26 (see check below); shootings & guns 115 -> 52;
# threats & stalking peaked at 90 in FY 2024-25, 58 in FY 2025-26; sexual
# assault 18-21 in last two years vs 5-14 earlier (small numbers).
...
police_all %>%
  ...
  pivot_wider(names_from = fy_label, values_from = n, values_fill = 0) %>%
  print(n = 20)
# Result: property crime less than half of FY 2019-20. Petty theft
# 676 -> 304, burglary 189 -> 66, car break-ins 416 -> 70 (FY 2024-25) ->
# 107, vehicle theft 189 -> 127, fraud & identity theft flat ~148.
...
# Result: genuine in the data. FY 2025-26's 15 robberies spread over 9 of
# 12 months (no gap); FY 2024-25 had 2-6 every month (46) plus 2
# carjackings. No renamed types -- only 211 ROBBERY (15) and 211A robbery
# ALARM (20, counted under Alarms). Small numbers; one year may be partly
# chance.
...
# Result: persons 3775, property 10753, parking 9256; 2019-01-01 to
# 2026-06-30.
...
# Result: 4 of 23784 (0.02%) -- no placeholder-midnight problem.
...
# Result (seasonal / trend strength): persons 0.44 / 0.37, property
# 0.26 / 0.81, parking 0.34 / 0.61. Seasonality weak to moderate; property
# is mostly a long decline.
# Peak/trough columns are calendar months with December shown as 0 (the
# index wraps modulo 12). feat_stl fits its own STL, so its peak can
# differ from the STL above (property: Oct here vs Dec below). Use the
# check below for the months reported.
...
weekday_share %>%
  ...
  print(n = 21)
# Result: persons -- no stable weekly pattern (shifts of 3-6 points year
# to year, ~500 incidents/yr). Property -- weekdays 14-17%, weekends
# 9-12%, steady. Parking -- follows the enforcement schedule, which
# changes by year: FY 2021-22 and FY 2025-26 nearly no weekend tickets
# (FY 2025-26 Sat 4.6%, Sun 3.2%); FY 2022-23 Friday 31%.
...
hour_share %>%
  ...
  summarize(busiest_hour  = hour[which.max(pct)],
            ...
            .groups = "drop") %>%
  print(n = 21)
# Result: persons lowest ~4 a.m., peak 3-5 p.m., same shape every year.
# Property jumps at 8 a.m., plateau to ~4 p.m. -- when thefts are
# reported, not necessarily when they happen (these are call times).
# Parking is a 7 a.m.-4 p.m. shift, sharp drop after 4 p.m.; FY 2019-20
# had more overnight tickets (~5% at midnight), recent years almost none.
...
monthly_stl %>%
  ...
  summarize(peak_month    = month_name[which.max(seasonal_effect)],
            ...
            .groups = "drop")
# Result (incidents per month above/below trend):
#   Persons  peak Sep +6.2,  trough Jul -7.2   (avg ~42/month, ~+/-15%)
#   Property peak Dec +10.9, trough Apr -10.4  (avg ~119/month, ~+/-9%)
#   Parking  peak Oct +40.5, trough Dec -37.4
# Caution: season(window = "periodic") assumes the same pattern every
# year. Part A found October parking erratic (39-274 by year), so the
# parking October peak is an average driven by a few years.

# ============================================================
# PHASE B16 -- PARKING ENFORCEMENT
# Its own section: parking tickets are not crimes, residents get them,
# and enforcement is in the news. Covers:
#   Trend -- tickets by fiscal year, and which months drove the
#            FY 2025-26 decline
#   Where -- top streets, landmark areas, and a parking-only map
#   When  -- the enforcement schedule by weekday and hour, by year
#   Plaza -- share of tickets near El Cerrito Plaza BART, now on a
#            fiscal-year basis for every year (B11 compared fiscal to
#            calendar years)
# Part A points use the corrected coordinates (Phase 7B).
# ============================================================

parking_points <- bind_rows(
  police_geocoded %>%
    filter(call_for_service == "PARKER - PARKING VIOLATION") %>%
    transmute(address, lat, long, BAD_LATLON_FLAG,
              dt = parse_date_time(received_date, orders = "mdy HM"),
              source = "2019-25"),
  police_fy26_geocoded %>%
    filter(call_for_service == "PARKER - PARKING VIOLATION") %>%
    transmute(address, lat, long, BAD_LATLON_FLAG, dt,
              source = "FY 2025-26")
) %>%
  filter(!is.na(dt)) %>%
  mutate(
    fy          = fiscal_year(dt),
    fy_label    = fy_label(fy),
    street_part = str_trim(str_extract(address, "^[^,]+")),
    street_name = street_part %>%
      str_remove("^\\d+[A-Z]?\\s+") %>%                # drop house number
      str_replace_all("\\bSPA\\b", "SAN PABLO AVE") %>%  # same expansions as B3
      str_replace_all("\\bPOT\\b", "POTRERO AVE") %>%
      str_squish()
  )

# --- CHECK: tickets per fiscal year match B11, and share mappable ---
parking_points %>%
  filter(fy >= 2020) %>%
  group_by(fy_label) %>%
  summarize(tickets = n(),
            mappable = sum(!BAD_LATLON_FLAG),
            pct_mappable = round(100 * mappable / tickets, 1),
            .groups = "drop")
# expect tickets 702 / 610 / 863 / 1054 / 1519 / 2317 / 1931
# Result: matches. 98-100% mappable every year (FY 2025-26: 1894 of 1931).
...
parking_fy %>%
  ...
  geom_text(aes(label = scales::comma(parking_violations)), vjust = -0.4, size = 3.5) +
  labs(title = "Parking tickets by fiscal year",
       subtitle = "El Cerrito Police Department call records",
       x = NULL, y = "Tickets") +
  theme_minimal()

# Which months drove the FY 2025-26 decline?
FY_MONTHS <- c("Jul", "Aug", "Sep", "Oct", "Nov", "Dec",
...
# Result: decline was lumpy, not steady. Biggest drops May (379 -> 164),
# Sep (233 -> 138), Apr (217 -> 129), Jan (169 -> 89); July (+100) and
# June (+79) rose. Enforcement comes in bursts.

# ------------------------------------------------------------
# WHERE
# ------------------------------------------------------------

# --- CHECK: street names are consistent across the two formats ---
# Part A and FY 2025-26 write addresses differently. Review the most
# common street names in each period for mismatches (e.g. "SAN PABLO"
# vs "SAN PABLO AVE").
parking_points %>%
  filter(fy >= 2025) %>%
  count(fy_label, street_name, sort = TRUE) %>%
  group_by(fy_label) %>%
  slice_head(n = 12) %>%
  print(n = 24)
# Result: street names consistent for single streets. Intersections are
# NOT comparable across formats: FY 2025-26 writes "HILL ST & LEXINGTON
# AVE", Part A shorthand like "HILL/LEXINGTON" is not combined with it.

# Top 15 FY 2025-26 streets, every fiscal year
top_streets_fy26 <- parking_points %>%
  filter(fy == 2026) %>%
...
parking_points %>%
  filter(fy >= 2020, street_name %in% top_streets_fy26) %>%
  count(street_name, fy_label) %>%
  pivot_wider(names_from = fy_label, values_from = n, values_fill = 0) %>%
  arrange(desc(`FY 2025-26`)) %>%
  print(n = 15)
# Result: San Pablo Ave 337 -> 180 (-157 of the -386 total decline).
# Rising: Liberty St 129 -> 136, Kearney St 72 -> 78, Lexington Ave
# 59 -> 74, Elm St 32 -> 45, Stockton Ave 9 -> 30. Intersection rows
# (e.g. HILL ST & LEXINGTON AVE 0 -> 45) reflect the format change.

# How concentrated is enforcement? Share of each year's tickets on its
# own top 5 streets.
parking_points %>%
  filter(fy >= 2020) %>%
  count(fy_label, street_name) %>%
  group_by(fy_label) %>%
  arrange(desc(n), .by_group = TRUE) %>%
  summarize(top5_pct = round(100 * sum(head(n, 5)) / sum(n), 1), .groups = "drop")
# Result: 24-33% every year (FY 2025-26 26.6%) -- not narrowing.

parking_usable <- parking_points %>% filter(!BAD_LATLON_FLAG)

# Landmarks: FY 2024-25 vs FY 2025-26, both full fiscal years
compute_landmark_counts(parking_usable %>% filter(fy == 2025), "fy_2024_25") %>%
  left_join(compute_landmark_counts(parking_usable %>% filter(fy == 2026), "fy_2025_26"),
            by = "landmark") %>%
  mutate(change = fy_2025_26 - fy_2024_25) %>%
  arrange(desc(fy_2025_26))
# Result (FY 2024-25 -> FY 2025-26): Del Norte BART 32 -> 80, Library
# 27 -> 52, ECHS 48 -> 37, EC Plaza BART 39 -> 33, Community Center
# 38 -> 16, Harding 38 -> 13. Enforcement shifted north and to the
# Library area.

# Plaza share, fiscal year basis for every year
parking_usable %>%
  filter(fy >= 2020) %>%
  mutate(dist_to_plaza_ft = distHaversine(cbind(long, lat),
                                          c(ec_plaza_bart$long, ec_plaza_bart$lat)) / 0.3048) %>%
  group_by(fy_label) %>%
  summarize(mappable = n(),
            near_plaza_1000ft = sum(dist_to_plaza_ft <= 1000),
            pct_near_plaza = round(100 * near_plaza_1000ft / mappable, 1),
            .groups = "drop")
# Result: 8.5 / 8.1 / 11.7 / 5.7 / 7.3 / 7.9 / 10.3% (FY 2019-20 to
# FY 2025-26). FY 2025-26's 196 tickets within 1,000 ft is the highest
# count of the seven years, while citywide tickets fell. Revises Part A's
# "not Plaza-specific" for the most recent year.

# Parking-only maps, FY 2024-25 and FY 2025-26 side by side
ggplot() +
  annotation_map_tile(type = "osm", zoomin = 0) +
...
# Enforcement schedule summary, every fiscal year
parking_points %>%
  filter(fy >= 2020) %>%
  group_by(fy_label) %>%
  summarize(tickets        = n(),
            weekend_pct    = round(100 * mean(wday(dt, week_start = 1) >= 6), 1),
            before_7am_pct = round(100 * mean(hour(dt) < 7), 1),
            after_4pm_pct  = round(100 * mean(hour(dt) >= 16), 1),
            .groups = "drop")
# Result (weekend / before 7 a.m. / after 4 p.m.):
#   FY 2019-20 27.4 / 24.9 / 24.5, FY 2020-21 21.0 / 16.9 / 19.3,
#   FY 2021-22 6.4 / 8.0 / 12.3,  FY 2022-23 23.9 / 11.5 / 11.4,
#   FY 2023-24 21.0 / 15.9 / 14.5, FY 2024-25 18.6 / 10.5 / 12.6,
#   FY 2025-26 7.8 / 7.5 / 7.3.
# FY 2019-20 enforcement was spread around the clock and week; FY 2025-26
# is concentrated on weekday daytime -- lowest before-7 a.m. and
# after-4 p.m. shares of the seven years.

# Weekday x hour heatmap, FY 2024-25 vs FY 2025-26
parking_points %>%
  filter(fy %in% c(2025, 2026)) %>%
  mutate(weekday = wday(dt, label = TRUE, week_start = 1),
         hour    = hour(dt)) %>%
  ...
  mutate(pct = 100 * n / sum(n)) %>%
  ungroup() %>%
  ggplot(aes(hour, fct_rev(weekday), fill = pct)) +
